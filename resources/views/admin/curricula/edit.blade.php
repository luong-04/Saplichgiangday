...
@section('title', 'Cập nhật Chương trình học')

@section('content')
<div class="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
    <div>
        <h1 class="text-2xl font-extrabold text-slate-800 tracking-tight">Cập nhật Chương trình học</h1>
        <p class="text-sm text-slate-500 font-medium mt-1">Sửa đổi phân bổ số tiết của môn học cho khối {{ $curriculum->grade }}</p>
    </div>
        ...
        <div class="space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <select id="grade" name="grade" required
                        ...
                        <option value="10" {{ old('grade', $curriculum->grade) == 10 ? 'selected' : '' }}>Khối 10</option>
                        <option value="11" {{ old('grade', $curriculum->grade) == 11 ? 'selected' : '' }}>Khối 11</option>
                        <option value="12" {{ old('grade', $curriculum->grade) == 12 ? 'selected' : '' }}>Khối 12</option>
                    </select>
                ...
                <div>
                    ...
                    <select id="subject_id" name="subject_id" required
                        ...
                        @foreach($subjects as $subject)
                            <option value="{{ $subject->id }}" data-lessons="{{ $subject->lessons_per_week }}" {{ old('subject_id', $curriculum->subject_id) == $subject->id ? 'selected' : '' }}>
            ...
            <div>
                ...
                <div class="relative w-1/2">
                    <input type="number" id="lessons_per_week" name="lessons_per_week" value="{{ old('lessons_per_week', $curriculum->lessons_per_week) }}" min="1" required
