...
<div class="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
    <div>
        <h1 class="text-2xl font-extrabold text-slate-800 tracking-tight">Cập nhật Môn học</h1>
        <p class="text-sm text-slate-500 font-medium mt-1">Chỉnh sửa thông tin và ràng buộc xếp lịch của môn học</p>
    </div>
            ...
            <div class="space-y-5">
                ...
                <div>
                    ...
                    <div class="flex gap-6 mt-2">
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="radio" name="type" value="1" class="w-4 h-4 text-blue-600 focus:ring-blue-500 border-slate-300" {{ old('type', $subject->type) == 1 ? 'checked' : '' }}>
                            <span class="text-sm font-medium text-slate-700">Lý thuyết</span>
                        </label>
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="radio" name="type" value="2" class="w-4 h-4 text-purple-600 focus:ring-purple-500 border-slate-300" {{ old('type', $subject->type) == 2 ? 'checked' : '' }}>
                ...
                <div>
                    <label class="flex items-center gap-3 cursor-pointer p-3 bg-amber-50 rounded-xl border border-amber-100 hover:bg-amber-100/50 transition-colors">
                        <input type="checkbox" name="is_fixed" value="1" class="w-5 h-5 text-amber-600 rounded border-amber-300 focus:ring-amber-500" {{ old('is_fixed', $subject->is_fixed) ? 'checked' : '' }}>
                        <div>
                ...
                <div>
                    ...
                    <select id="room_category_id" name="room_category_id"
                        ...
                        @foreach($roomCategories as $cat)
                            <option value="{{ $cat->id }}" {{ old('room_category_id', $subject->room_category_id) == $cat->id ? 'selected' : '' }}>
                                {{ $cat->name }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="space-y-5">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" id="lessons_per_week" name="lessons_per_week" value="{{ old('lessons_per_week', $subject->lessons_per_week) }}" min="1" required
                    ...
                    <div>
                        ...
                        <input type="number" id="max_lessons_per_day" name="max_lessons_per_day" value="{{ old('max_lessons_per_day', $subject->max_lessons_per_day) }}" min="1" max="5" required
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" id="consecutive_periods" name="consecutive_periods" value="{{ old('consecutive_periods', $subject->consecutive_periods) }}" min="1" max="5" required
                    ...
                    <div>
                        <label class="block text-sm font-bold text-slate-700 mb-1" for="max_periods_per_day">Tối đa dạy/ngày/GV <span class="text-red-500">*</span></label>
                        <input type="number" id="max_periods_per_day" name="max_periods_per_day" value="{{ old('max_periods_per_day', $subject->max_periods_per_day) }}" min="1" max="10" required
